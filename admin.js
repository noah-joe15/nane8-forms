...
(function () {
  ...
  // FIXED: Only ONE renderPercentageBreakdowns function exists now
  function renderPercentageBreakdowns(submissions) {
    ...
    function buildBreakdown(data, field, limit) {
      ...
      data.forEach(function (s) { 
        ...
        if (!v || v.trim() === '') return; // Skip empty values
        v = v.trim().charAt(0).toUpperCase() + v.trim().slice(1).toLowerCase(); // Normalize
...
var BRAND = {
  blue: [11, 61, 145], blueDark: [6, 40, 100], blueLight: [230, 240, 255],
  green: [0, 104, 71], greenDark: [0, 60, 40], greenLight: [220, 245, 235],
  gold: [212, 160, 23], goldLight: [255, 248, 220], white: [255, 255, 255],
  black: [31, 41, 55], gray: [107, 114, 128], grayLight: [243, 244, 246],
  red: [220, 38, 38], redLight: [254, 226, 226], amber: [217, 119, 6], amberLight: [254, 243, 199]
};

function drawGradientRect(doc, x, y, w, h, colorTop, colorBottom) {
  var steps = 20; var stepH = h / steps;
...
function drawKpiCard(doc, x, y, w, h, value, label, icon, color) {
  ...
  doc.setDrawColor(229, 231, 235); doc.setLineWidth(0.3); doc.roundedRect(x, y, w, h, 3, 3, 'S');
  doc.setFillColor(color[0], color[1], color[2]); doc.rect(x, y, w, 2, 'F');
  doc.setFillColor(color[0], color[1], color[2]); doc.circle(x + 10, y + 14, 5, 'F');
  doc.setTextColor(255, 255, 255); doc.setFontSize(8); doc.setFont('helvetica', 'bold');
  doc.text(icon, x + 10, y + 16, { align: 'center' });
  doc.setTextColor(31, 41, 55); doc.setFontSize(22); doc.setFont('helvetica', 'bold');
  doc.text(String(value), x + w / 2, y + 22, { align: 'center' });
  doc.setTextColor(107, 114, 128); doc.setFontSize(8); doc.setFont('helvetica', 'normal');
  doc.text(doc.splitTextToSize(label, w - 6), x + w / 2, y + 30, { align: 'center' });
}

function drawInsightBox(doc, x, y, w, type, category, text) {
  var colors = { critical: { bg: BRAND.redLight, accent: BRAND.red, text: BRAND.red }, warning: { bg: BRAND.amberLight, accent: BRAND.amber, text: BRAND.amber }, success: { bg: BRAND.greenLight, accent: BRAND.green, text: BRAND.green }, info: { bg: BRAND.blueLight, accent: BRAND.blue, text: BRAND.blue } };
  var c = colors[type] || colors.info; var boxH = 18;
  doc.setFillColor(c.bg[0], c.bg[1], c.bg[2]); doc.roundedRect(x, y, w, boxH, 2, 2, 'F');
  doc.setFillColor(c.accent[0], c.accent[1], c.accent[2]); doc.rect(x, y, 1.5, boxH, 'F');
  doc.setTextColor(c.text[0], c.text[1], c.text[2]); doc.setFont('helvetica', 'bold'); doc.setFontSize(8);
  doc.text(category.toUpperCase(), x + 5, y + 5);
  doc.setTextColor(31, 41, 55); doc.setFont('helvetica', 'normal'); doc.setFontSize(9);
  doc.text(doc.splitTextToSize(text, w - 10), x + 5, y + 11);
  return boxH + 3;
}

function drawSectionHeader(doc, y, number, title, subtitle) {
  var margin = 15; var pageWidth = doc.internal.pageSize.getWidth();
  doc.setFillColor(BRAND.green[0], BRAND.green[1], BRAND.green[2]); doc.rect(margin, y, 4, 12, 'F');
  doc.setTextColor(BRAND.green[0], BRAND.green[1], BRAND.green[2]); doc.setFont('helvetica', 'bold'); doc.setFontSize(24);
  doc.text(number, margin + 8, y + 9);
  doc.setTextColor(BRAND.blueDark[0], BRAND.blueDark[1], BRAND.blueDark[2]); doc.setFontSize(16);
  doc.text(title, margin + 22, y + 8);
  if (subtitle) { doc.setTextColor(BRAND.gray[0], BRAND.gray[1], BRAND.gray[2]); doc.setFont('helvetica', 'normal'); doc.setFontSize(9); doc.text(subtitle, margin + 22, y + 14); }
  doc.setDrawColor(229, 231, 235); doc.setLineWidth(0.3); doc.line(margin, y + 18, pageWidth - margin, y + 18);
  return y + 24;
}

function drawRecommendationCard(doc, x, y, w, idx, category, action, impact, timeline) {
  var impactColors = { 'High': BRAND.red, 'Medium': BRAND.amber, 'Low': BRAND.green };
  var impactColor = impactColors[impact] || BRAND.gray; var cardH = 42;
  doc.setFillColor(255, 255, 255); doc.roundedRect(x, y, w, cardH, 3, 3, 'F');
  doc.setDrawColor(229, 231, 235); doc.setLineWidth(0.5); doc.roundedRect(x, y, w, cardH, 3, 3, 'S');
  doc.setFillColor(BRAND.blue[0], BRAND.blue[1], BRAND.blue[2]); doc.rect(x, y, 3, cardH, 'F');
  doc.setFillColor(BRAND.blue[0], BRAND.blue[1], BRAND.blue[2]); doc.circle(x + 14, y + 12, 6, 'F');
  doc.setTextColor(255, 255, 255); doc.setFont('helvetica', 'bold'); doc.setFontSize(10);
  doc.text(String(idx), x + 14, y + 14, { align: 'center' });
  doc.setTextColor(BRAND.blue[0], BRAND.blue[1], BRAND.blue[2]); doc.setFont('helvetica', 'bold'); doc.setFontSize(9);
  doc.text(category.toUpperCase(), x + 26, y + 9);
  var badgeW = 24; var badgeH = 9; var badgeX = x + w - badgeW - 5;
  doc.setFillColor(impactColor[0], impactColor[1], impactColor[2]); doc.roundedRect(badgeX, y + 5, badgeW, badgeH, 2, 2, 'F');
  doc.setTextColor(255, 255, 255); doc.setFont('helvetica', 'bold'); doc.setFontSize(7);
  doc.text(impact.toUpperCase(), badgeX + badgeW / 2, y + 11, { align: 'center' });
  doc.setTextColor(31, 41, 55); doc.setFont('helvetica', 'normal'); doc.setFontSize(9);
  doc.text(doc.splitTextToSize(action, w - 70), x + 26, y + 18);
  doc.setTextColor(BRAND.gray[0], BRAND.gray[1], BRAND.gray[2]); doc.setFont('helvetica', 'normal'); doc.setFontSize(8);
  var clockCx = x + 32; var clockCy = y + cardH - 8; var clockR = 3.5;
  doc.setDrawColor(BRAND.gray[0], BRAND.gray[1], BRAND.gray[2]); doc.setLineWidth(0.4);
  doc.circle(clockCx, clockCy, clockR, 'S'); doc.line(clockCx, clockCy, clockCx, clockCy - 1.8); doc.line(clockCx, clockCy, clockCx + 1.8, clockCy);
  doc.text('Timeline: ' + timeline, x + 40, y + cardH - 6);
  return cardH + 5;
}

function generateSmartRecommendations(data) {
  var recommendations = []; var insights = [];
  ...
  if (parseFloat(sellingRawPercentage) > 60) {
    insights.push({ type: 'critical', category: 'Raw Export Alert', text: sellingRawPercentage + '% of stakeholders sell raw materials without value addition.' });
    recommendations.push({ category: 'Policy & Regulation', action: 'Implement mandatory local processing requirements before export permits.', impact: 'High', timeline: '6-12 months' });
  } else if (parseFloat(sellingRawPercentage) > 30) {
    insights.push({ type: 'warning', category: 'Processing Gap', text: sellingRawPercentage + '% of products are still sold raw.' });
  } else {
    insights.push({ type: 'success', category: 'Processing Performance', text: 'Only ' + sellingRawPercentage + '% of products are sold raw.' });
  }
  ...
  if (underutilized > totalResponses * 0.4) {
    insights.push({ type: 'critical', category: 'Factory Utilization', text: 'Over ' + underutilized + ' factories are underutilizing capacity.' });
    recommendations.push({ category: 'Investment', action: 'Improve raw material supply chains through contract farming.', impact: 'High', timeline: '3-6 months' });
  }
  ...
  if (financeIssues > totalResponses * 0.5) {
    insights.push({ type: 'critical', category: 'Financing Constraints', text: 'Over ' + financeIssues + ' stakeholders cite high interest rates.' });
    recommendations.push({ category: 'Financial Services', action: 'Establish a dedicated value-addition fund offering subsidized loans.', impact: 'High', timeline: '6-12 months' });
  }
  
  var regions = {}; data.forEach(function(r) { if (r.mkoa) regions[r.mkoa] = (regions[r.mkoa] || 0) + 1; });
  var regionCount = Object.keys(regions).length;
  insights.push({ type: 'info', category: 'Geographic Reach', text: 'Data collected from ' + regionCount + ' regions.' });
  
  recommendations.push({ category: 'Capacity Building', action: 'Launch training programs on processing technology and quality standards.', impact: 'Medium', timeline: '3-6 months' });
  
  return { insights: insights, recommendations: recommendations, statistics: { totalResponses: totalResponses, sellingRawPercentage: sellingRawPercentage, regionCount: regionCount, underutilizedFactories: underutilized } };
}
...
function categorizeResponses(data) {
  ...
  data.forEach(function(r) {
    ...
    if (r['bidhaa[]']) { var bids = Array.isArray(r['bidhaa[]']) ? r['bidhaa[]'] : [r['bidhaa[]']]; bids.forEach(function(b) { categories.byCommodity[b] = (categories.byCommodity[b] || 0) + 1; }); }
    if (r['aina_mdau[]']) { var types = Array.isArray(r['aina_mdau[]']) ? r['aina_mdau[]'] : [r['aina_mdau[]']]; types.forEach(function(t) { categories.byStakeholderType[t] = (categories.byStakeholderType[t] || 0) + 1; }); }
...
window.generateWadauMalighafiReport = async function() {
  ...
  var margin = 15; var contentWidth = pageWidth - 2 * margin;
  ...
  doc.setFillColor(255, 255, 255); doc.rect(0, 0, pageWidth, 8, 'F');
  doc.setFillColor(BRAND.green[0], BRAND.green[1], BRAND.green[2]); doc.rect(0, 8, pageWidth, 2, 'F');
  doc.setTextColor(255, 255, 255); doc.setFont('helvetica', 'bold'); doc.setFontSize(14);
  doc.text('TANTRADE', margin, 20);
  doc.setFont('helvetica', 'normal'); doc.setFontSize(8); doc.text('Tanzania Trade Development Authority', margin, 26);
  doc.setFont('helvetica', 'bold'); doc.setFontSize(28); doc.text('MARKET ASSESSMENT', margin, 80); doc.text('REPORT 2026', margin, 92);
  doc.setFillColor(BRAND.gold[0], BRAND.gold[1], BRAND.gold[2]); doc.rect(margin, 98, 60, 2, 'F');
  doc.setFont('helvetica', 'normal'); doc.setFontSize(12); doc.text('Top 10 Commodities Exported as Raw Materials', margin, 110);
  
  var cardY = 180; doc.setFillColor(255, 255, 255); doc.roundedRect(margin, cardY, contentWidth, 50, 3, 3, 'F');
  doc.setTextColor(BRAND.blue[0], BRAND.blue[1], BRAND.blue[2]); doc.setFont('helvetica', 'bold'); doc.setFontSize(10);
  doc.text('REPORT DETAILS', margin + 5, cardY + 10);
  doc.setTextColor(31, 41, 55); doc.setFont('helvetica', 'normal'); doc.setFontSize(9);
  doc.text('Total Responses: ' + analysis.statistics.totalResponses, margin + 5, cardY + 20);
  doc.text('Regions Covered: ' + analysis.statistics.regionCount, margin + 5, cardY + 28);
  ...
  doc.text('This report presents a comprehensive market assessment of Tanzania\'s top 10 agricultural and mineral commodities exported as raw materials.', margin, y + 5);
  
  var kpiW = (contentWidth - 12) / 3; var kpiH = 38;
  drawKpiCard(doc, margin, y + 15, kpiW, kpiH, analysis.statistics.totalResponses, 'Total Responses', 'R', BRAND.blue);
  drawKpiCard(doc, margin + kpiW + 6, y + 15, kpiW, kpiH, analysis.statistics.sellingRawPercentage + '%', 'Selling Raw', '%', BRAND.red);
  drawKpiCard(doc, margin + 2 * (kpiW + 6), y + 15, kpiW, kpiH, analysis.statistics.regionCount, 'Regions', 'G', BRAND.green);
  
  doc.addPage();
  y = drawSectionHeader(doc, 20, '03', 'KEY INSIGHTS', 'Critical Observations');
  analysis.insights.forEach(function(insight) { if (y > pageHeight - 35) { doc.addPage(); y = 20; } y += drawInsightBox(doc, margin, y, contentWidth, insight.type, insight.category, insight.text); });
  
  doc.addPage();
  y = drawSectionHeader(doc, 20, '04', 'REGIONAL DISTRIBUTION', 'Geographic Breakdown');
  var regionData = Object.keys(categories.byRegion).map(function(r) { return [r, categories.byRegion[r], ((categories.byRegion[r] / analysis.statistics.totalResponses) * 100).toFixed(1) + '%']; }).sort(function(a, b) { return parseInt(b[1]) - parseInt(a[1]); });
  doc.autoTable({ startY: y, margin: { left: margin, right: margin }, head: [['Region', 'Responses', 'Share']], body: regionData.slice(0, 10), styles: { font: 'helvetica', fontSize: 9, cellPadding: 3 }, headStyles: { fillColor: BRAND.blue, textColor: 255, fontStyle: 'bold', halign: 'center' }, columnStyles: { 0: { cellWidth: 80 }, 1: { cellWidth: 40, halign: 'center' }, 2: { cellWidth: 40, halign: 'right' } }, alternateRowStyles: { fillColor: BRAND.grayLight }, theme: 'grid' });
  
  doc.addPage();
  y = drawSectionHeader(doc, 20, '08', 'STRATEGIC RECOMMENDATIONS', 'AI-Generated Action Plan');
  analysis.recommendations.forEach(function(rec, idx) { if (y > pageHeight - 45) { doc.addPage(); y = 20; } y += drawRecommendationCard(doc, margin, y, contentWidth, idx + 1, rec.category, rec.action, rec.impact, rec.timeline); });
  
  var pageCount = doc.getNumberOfPages();
  for (var i = 1; i <= pageCount; i++) {
    doc.setPage(i); if (i === 1) continue;
    doc.setFillColor(BRAND.blueDark[0], BRAND.blueDark[1], BRAND.blueDark[2]); doc.rect(0, pageHeight - 12, pageWidth, 12, 'F');
    doc.setFillColor(BRAND.green[0], BRAND.green[1], BRAND.green[2]); doc.rect(0, pageHeight - 12, pageWidth, 1.5, 'F');
