...
(function () {
  ...
  var processingChart, productionTrendChart, businessScaleChart;
  ...
  function renderProductKPIs() {
    ...
    allSubmissions.forEach(function(sub) {
      ...
      productsArray.forEach(function(product) {
        ...
        // Try NEW dynamic fields first (kiasi_Dhahabu, thamani_Dhahabu, etc.)
        ...
        // If NEW fields are empty, try OLD fields (kiasi_2024, thamani_2024, kipimo_2024)
        // This handles backward compatibility for old submissions
        if (qty === 0 && value === 0) {
          var oldQty = parseFloat(sub['kiasi_2024']) || 0;
          var oldValue = parseFloat(sub['thamani_2024']) || 0;
          var oldUnit = sub['kipimo_2024'] || '';
          
          if (oldQty > 0 || oldValue > 0) {
            // Distribute old values equally among selected products
            qty = oldQty / productsArray.length;
            value = oldValue / productsArray.length;
            unit = oldUnit;
          }
        }
  ...
  function updateCharts() {
    ...
    var processing = {}, productionTrend = {}, businessScale = {};
  ...
  var BRAND = {
    blue: [11, 61, 145],
    blueDark: [6, 40, 100],
    blueLight: [230, 240, 255],
    green: [0, 104, 71],
    greenDark: [0, 60, 40],
    greenLight: [220, 245, 235],
    gold: [212, 160, 23],
  ...
  function drawRecommendationCard(doc, x, y, w, idx, category, action, impact, timeline) {
    ...
    doc.circle(clockCx, clockCy, clockR, 'S');
    doc.line(clockCx, clockCy, clockCx, clockCy - 1.8);
    doc.line(clockCx, clockCy, clockCx + 1.8, clockCy);
  ...
  function generateProductKPITableData() {
    ...
    return sortedProducts.map(function(product) {
      ...
      return [
        product,
        data.totalQty.toLocaleString('en-TZ') + ' ' + (data.unit || ''),
        'TSh ' + data.totalValue.toLocaleString('en-TZ'),
        data.count,
        (data.count > 0 ? (data.totalValue / data.count).toLocaleString('en-TZ', {maximumFractionDigits: 0}) : 0)
      ];
    });
  }
